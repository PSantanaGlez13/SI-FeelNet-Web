...
flags.DEFINE_string("output_data", "data/new_train.csv",
...
def replace_labels(labels, idx2emotion, mapping_dict, emotion2idx):
  ...
  split = labels.split(",")
  new_labels = []
  added_emotions = set()  # Set to track added emotions
  ...
  for label_idx in split:
    ...
    for new_emotion, v in mapping_dict.items():
      if old_emotion in v:
        new_emotion_id = str(emotion2idx[new_emotion])
        if new_emotion_id not in added_emotions:
          new_labels.append(new_emotion_id)
          added_emotions.add(new_emotion_id)  # Mark as added
        found = True
        break
    if not found:
        new_emotion_id = str(emotion2idx[old_emotion])
        if new_emotion_id not in added_emotions:
            new_labels.append(new_emotion_id)
            added_emotions.add(new_emotion_id)  # Mark as added
...
def main(_):
  ...
  # Filter rows where the second column (labels) has only one emotion
  filtered_data = data[data['labels'].apply(lambda x: len(x.split(",")) == 1)]

  filtered_data.to_csv(
      FLAGS.output_data, sep=",", encoding="utf-8", header=True, index=False)
